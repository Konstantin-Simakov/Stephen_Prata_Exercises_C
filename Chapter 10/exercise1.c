...
int main (void)
{
	...
	{	
		/* for each year, summarize the amount of precipitation for each month */
		for (month = 0, subtot = 0; month < MONTHS; month++)
			subtot += *(*(rain + year) + month);
		printf("%d %15.1f\n", 2010 + year, subtot);
		total += subtot;
	}
	printf("The average rainfall is %.1f inches.\n\n", 
		total / YEARS);
	...
	{
		/* for each month, summarize the amount of precipitation over the years */
